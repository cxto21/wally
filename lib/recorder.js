...
(function() {
  ...
  if (!window.__wally_polling_started) {
    ...
    function _getFingerprint() {
      ...
      // Visible buttons
      var btns = document.querySelectorAll('button, [role="button"], a[role="button"]');
      for (var i = 0; i < btns.length; i++) {
        var b = btns[i];
        var rect = b.getBoundingClientRect();
        if (rect.width > 0 && rect.height > 0) {
          var t = (b.textContent || '').trim().substring(0, 40);
          if (t) parts.push('btn:' + t);
        }
      }
      // Input values
      ...
      for (var k = 0; k < headings.length; k++) {
        ...
        if (hr.width > 0 && hr.height > 0) {
          parts.push('h:' + (h.textContent || '').trim().substring(0, 40));
        }
      }
      return parts.join('|');
    }
    ...
    setInterval(function() {
      try {
        ...
        // Diff: find new buttons that appeared
        var prevBtns = prev.split('|').filter(function(p) { return p.startsWith('btn:'); });
        var currBtns = fp.split('|').filter(function(p) { return p.startsWith('btn:'); });
        var prevSet = {};
        prevBtns.forEach(function(b) { prevSet[b] = true; });
        currBtns.forEach(function(b) {
          if (!prevSet[b]) {
            var text = b.substring(4);
            window.__wally_actions.push({ type: 'click_detected', selector: 'button "' + text + '"', text: text });
          }
        });

        // Diff: find removed buttons (user clicked them)
        var currSet = {};
