...
async function attachNetworkCapture(cdpSession, networkFile) {
  ...
  // Ensure file exists
  try { if (!fs.existsSync(networkFile)) fs.writeFileSync(networkFile, ''); } catch (e) { log.debug(`network file init: ${e.message}`); }

  function append(type, params) {
    const entry = { ts: new Date().toISOString(), type, params };
    try { fs.appendFileSync(networkFile, JSON.stringify(entry) + '\n'); } catch (e) { log.debug(`network append: ${e.message}`); }
  }
  ...
  return async () => {
    for (const [evt, handler] of Object.entries(handlers)) {
      try { cdpSession.off(evt, handler); } catch (e) { log.debug(`off ${evt}: ${e.message}`); }
    }
    try { await cdpSession.send('Network.disable').catch(() => {}); } catch (e) { log.debug(`Network.disable: ${e.message}`); }
  };
}
...
function buildEntry(p, ts) {
  ...
  } catch (e) { log.debug(`URL parse: ${e.message}`); }
