...
const CHROME_DATA_DIR = process.env.CHROME_DATA_DIR || '/tmp/opencode/chrome-cdp';
...
function killChrome() {
  ...
  } catch (e) { log.debug(`killChrome: ${e.message}`); }
}
...
function setupChromeDataDir(profileName) {
  ...
  if (!fs.existsSync(dstProfile)) {
    try { fs.symlinkSync(srcProfile, dstProfile); } catch (e) { log.debug(`symlink profile: ${e.message}`); }
  }

  // Copy Local State (needed for profile discovery)
  const srcLocalState = path.join(os.homedir(), '.config/google-chrome', 'Local State');
...
async function ensureCDP(profileName, url, retries = 60) {
  ...
  const hasChrome = execSync('pgrep -f "chrome" || true', { encoding: 'utf8' }).trim();
  if (hasChrome) {
