  ...
  for (const action of actions) {
    ...
    if (action.type === 'navigate') {
      ...
      if (sel.includes('password')) {
        ...
        // Text-based selector for generic elements (div, span, p with text)
        const text = sel.match(/"(.+)"/)?.[1] || '';
        const v = `_txt${Math.random().toString(36).substring(2,6)}`;
        test += `${indent}{ const ${v} = ${target}.getByText(${JSON.stringify(text)}, { exact: false }).first(); if (await ${v}.isVisible().catch(()=>false)) { try { await ${v}.click({ timeout: 10000 }); } catch(e) { console.log('[Wally] Text click failed (continuing):', e.message.split(String.fromCharCode(10))[0]); } } else { console.log('[Wally] Skip text not visible: ' + ${JSON.stringify(text)}); } }\n`;
      ...
      if (action.position) {
        // Position-based: double-click at coordinates (generic, works with any text)
        test += `${indent}await ${target}.mouse.click(${action.position.x}, ${action.position.y}, { clickCount: 2 });\n`;
      } else {
        const dblSel = action.selector.startsWith('text "')
          ? `text=${action.selector.match(/"(.+)"/)?.[1] || ''}`
          : action.selector;
        test += `${indent}{ const _dbl = ${target}.locator('${dblSel}').first(); if (await _dbl.isVisible().catch(()=>false)) { try { await _dbl.dblclick({ timeout: 10000 }); } catch(e) { console.log('[Wally] Dblclick failed (continuing):', e.message.split(String.fromCharCode(10))[0]); } } else { console.log('[Wally] Skip dblclick not visible: ${action.selector}'); } }\n`;
      }
      test += `${indent}await page.waitForTimeout(500);\n`;
      ...
      if (action.position) {
        // Position-based: click at coordinates then Ctrl+A Ctrl+C
        test += `${indent}await ${target}.mouse.click(${action.position.x}, ${action.position.y});\n`;
        test += `${indent}await ${target}.keyboard.press('Control+a');\n`;
        test += `${indent}await ${target}.keyboard.press('Control+c');\n`;
      } else {
        const copySel = action.selector.startsWith('text "')
          ? `text=${action.selector.match(/"(.+)"/)?.[1] || ''}`
          : action.selector;
        test += `${indent}{ const _src = ${target}.locator('${copySel}').first(); if (await _src.isVisible().catch(()=>false)) { try { await _src.click({ timeout: 5000 }); await ${target}.keyboard.press('Control+a'); await ${target}.keyboard.press('Control+c'); } catch(e) { console.log('[Wally] Copy failed (continuing):', e.message.split(String.fromCharCode(10))[0]); } } else { console.log('[Wally] Skip copy source not visible: ${action.selector}'); } }\n`;
      }
