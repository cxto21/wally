...
/**
 * In-memory store for live actions streamed during recording.
 * Keyed by session id. Accumulated via POST /actions, retrieved via GET /actions.
...
function createServer(port) {
  ...
  const server = http.createServer((req, res) => {
    ...
    // POST /actions?id=<session-id> — live action accumulation
    if (req.method === 'POST' && req.url.startsWith('/actions?')) {
      ...
      req.on('end', () => {
        try {
          ...
          const actions = Array.isArray(payload) ? payload : (payload.actions ? payload.actions : [payload]);

          if (!liveActions.has(sessionId)) {
            liveActions.set(sessionId, []);
          }
          const stored = liveActions.get(sessionId);
          for (const action of actions) {
            stored.push(action);
          }

          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: true, count: stored.length }));
        } catch (e) {
          res.writeHead(400, { 'Content-Type': 'application/json' });
    ...
    // GET /actions?id=<session-id> — retrieve accumulated live actions
      ...
      const actions = liveActions.get(sessionId) || [];
      res.writeHead(200, { 'Content-Type': 'application/json' });
    ...
    // POST /session — receive session from extension
      ...
      req.on('end', () => {
        try {
          ...
          // Merge live actions into session if available
          const sid = session.id || '';
          const live = liveActions.get(sid);
          if (live && live.length > 0) {
            session.actions = (session.actions || []).concat(live);
            // Sort by timestamp for correct ordering
            session.actions.sort((a, b) => (a.ts || '').localeCompare(b.ts || ''));
            // Clean up live actions after merge
            liveActions.delete(sid);
          }
