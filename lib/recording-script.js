/**
 * Wally Recording Script — injected into pages to capture user interactions.
...
(function() {
  ...
  window.__wally_resolveSelector = (el) => {
    ...
    // 9. Elements with short unique text (good for dblclick/select text)
  ...
  // COPY — capture copied text and source element
  document.addEventListener('copy', function(e) {
    var el = deepEventTarget(e);
    if (!el || el.nodeType !== Node.ELEMENT_NODE) return;
    var selector = window.__wally_resolveSelector(el);
    var selectedText = '';
    try { selectedText = window.getSelection().toString(); } catch (err) {}
    if (!selectedText) {
      try { selectedText = el.value || el.textContent || ''; } catch (err) {}
    }
    // Use position-based selector for generic copy (works with any text)
    var pos = null;
    try {
      var r = window.getSelection().getRangeAt(0);
      var rect = r.getBoundingClientRect();
      pos = { x: Math.round(rect.x + rect.width / 2), y: Math.round(rect.y + rect.height / 2) };
    } catch (err) {}
    window.__wally_actions.push({
      type: 'copy',
      selector: pos ? 'position' : selector,
      position: pos,
      text: selectedText.substring(0, 200).trim(),
    });
  }, true);

  // PASTE — capture paste target element
  document.addEventListener('paste', function(e) {
    var el = deepEventTarget(e);
    if (!el || el.nodeType !== Node.ELEMENT_NODE) return;
    var selector = window.__wally_resolveSelector(el);
    var pastedText = '';
    try { pastedText = (e.clipboardData || window.clipboardData).getData('text'); } catch (err) {}
    window.__wally_actions.push({
      type: 'paste',
      selector: selector,
      text: pastedText.substring(0, 200).trim(),
    });
  }, true);
...
module.exports = { RECORDING_SCRIPT, PRE_NAVIGATE_SCRIPT };
