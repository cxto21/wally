      ...
      try {
        ...
        for (const action of actions) {
          ...
          if (ap !== lastPage && ap.startsWith('ext:')) {
            ...
            test += `  if (extPage) { await extPage.bringToFront().catch(()=>{}); await extPage.waitForLoadState('domcontentloaded').catch(()=>{}); await extPage.waitForTimeout(1500); }\n`;
            lastPage = ap;
          } else if (ap !== lastPage && ap === 'main') {
            test += `\n  page = context.pages().find(p => !p.url().startsWith('chrome-extension://')) || context.pages()[0];\n`;
            lastPage = ap;
          }
          const indent = (lastPage.startsWith('ext:') ? '    ' : '  ');
          if (action.type === 'navigate') {
            test += `${indent}await page.goto('${action.url}', {waitUntil:'domcontentloaded',timeout:30000});\n${indent}await page.waitForTimeout(2000);\n`;
          } else if (action.type === 'click') {
            const sel = action.selector; const target = (lastPage.startsWith('ext:') ? 'extPage' : 'page');
            if (sel.startsWith('[data-testid=') || sel.startsWith('#') || sel.startsWith('[aria-label=')) {
              ...
              if (isTextOnly) test += `${indent}// Skipped: ${sel} "${txt.substring(0,30)}"\n`;
              else if (isWallet && txt) test += `${indent}await ${target}.getByText(/${txt.replace(/[.*+?^${}()|[\]\\]/g,'\\$&').substring(0,30)}/i).first().click({timeout:5000});\n`;
              else test += `${indent}await ${target}.locator('${sel}').first().click({force:true,timeout:5000});\n`;
            } else {
          ...
          if (next && lastPage.startsWith('ext:') && (next.page||'main')==='main') test += `  }\n\n`;
        }
