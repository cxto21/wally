          ...
          if (action.type === 'navigate') {
            ...
            if (sel.includes('password')) {
              ...
              if (isTextOnly || isErrorOverlay) test += `${indent}// Skipped: ${sel} "${txt.substring(0,30).replace(/\n/g,' ')}"\n`;
                ...
                test += `${indent}{ const ${v}=${target}.getByText(/${esc}/i).first(); if(await ${v}.isVisible().catch(()=>false)) { try { await ${v}.click({force:true,timeout:10000}); } catch(e) { console.log('[Wally] Wallet click failed (continuing):', e.message.split('\\n')[0]); } } else console.log('[Wally] Skip wallet not visible: ${esc}'); }\n`;
