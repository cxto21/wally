          ...
          if (action.type === 'navigate') {
            ...
            if (sel.includes('password')) {
              ...
              if (isTextOnly) test += `${indent}// Skipped: ${sel} "${txt.substring(0,30)}"\n`;
                ...
                test += `${indent}{ const ${v}=${target}.getByText(/${esc}/i).first(); if(await ${v}.isVisible().catch(()=>false)) await ${v}.click({timeout:5000}); else console.log('[Wally] Skip wallet not visible: ${esc}'); }\n`;
