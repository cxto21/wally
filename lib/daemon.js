        ...
        for (const action of actions) {
          ...
          if (action.type === 'navigate') {
            ...
            if (sel.includes('password')) {
              ...
              const txt = action.text||''; const isExtension = ['Ready','Argent','Wallet'].some(k=>txt.includes(k));
              const isErrorOverlay = txt.includes('Contrase') || txt.includes('Loading') || txt.includes('Bloq May') || txt.includes('Desbloque');
              const isTextOnly = (sel==='p' || sel.endsWith(' > p') || (sel.endsWith(' > span') && !sel.includes('button'))) && txt.length>15 && !isExtension;
              if (isTextOnly || isErrorOverlay) test += `${indent}// Skipped: ${sel} "${txt.substring(0,30).replace(/\n/g,' ')}"\n`;
              else if (isExtension && txt) {
                const esc = txt.replace(/[.*+?^${}()|[\]\\]/g,'\\$&').substring(0,30);
                const v = `_ext${Math.random().toString(36).substring(2,6)}`;
                test += `${indent}{ const ${v}=${target}.getByText(/${esc}/i).first(); if(await ${v}.isVisible().catch(()=>false)) { try { await ${v}.click({force:true,timeout:10000}); } catch(e) { console.log('[Wally] Extension click failed (continuing):', e.message.split(String.fromCharCode(10))[0]); } } else console.log('[Wally] Skip extension not visible: ' + ${JSON.stringify(esc)}); }\n`;
