...
class WallyDaemon {
  ...
  async start(options = {}) {
    ...
    this.sessionDir = path.join(SESSIONS_DIR, 'daemon-' + Date.now());
  ...
  async attachToTarget(targetId, url) {
    try {
      ...
      if (targetPage) {
        ...
        console.log(`[Wally Daemon] Could not find page for ${label}, will retry on next poll`);
      }
   ...
   * Also polls actions via CDP since page.evaluate won't work on extension pages.
   */
  async injectViaCDP(page, sessionId) {
    const { RECORDING_SCRIPT } = require('./recorder');
    try {
      // Use Playwright's underlying CDP session to evaluate in the page context
      const cdp = await page.context().newCDPSession(page);
      await cdp.send('Runtime.evaluate', {
        expression: RECORDING_SCRIPT,
        includeCommandLineAPI: false,
      });
      console.log(`[Wally Daemon] Injected recording script via CDP (bypass CSP)`);
    } catch (e) {
