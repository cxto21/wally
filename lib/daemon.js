...
class WallyDaemon {
  constructor() {
    ...
    this.initialPages = new Set(); // pages that existed when daemon started — don't auto-record
  }
  ...
  async start(options = {}) {
    ...
    // Track pages that exist at startup — discoverPages won't auto-record these
    for (const p of ctx.pages()) {
      const u = p.url();
      if (u && !u.startsWith('chrome://') && !u.startsWith('about:')) {
        this.initialPages.add(u);
      }
    }
    console.log(`[Wally Daemon] Initial pages: ${[...this.initialPages].map(u => new URL(u).hostname).join(', ') || 'none'}`);
  ...
  async onTargetCreated(event) {
    ...
    if (this.extAux) {
      ...
      if (!isExt) return;
      if (this.isWallyExtension(targetInfo.url)) return;
    }

    console.log(`[Wally Daemon] Target created: ${targetInfo.targetId} type=${targetInfo.type} url=${(targetInfo.url || 'about:blank').substring(0, 60)}`);

    // Wait for page to initialize (new tabs start with about:blank)
    await new Promise(r => setTimeout(r, 800));

    // Re-check URL after delay — tab might have loaded by now
    const url = targetInfo.url || '';
    if (!url || url === 'about:blank' || url.startsWith('chrome-devtools://')) {
      // Tab still blank — discoverPages will pick it up later
      log.debug(`Target ${targetInfo.targetId} still blank after wait, deferring to discoverPages`);
      return;
    }

    await this.attachToTarget(targetInfo.targetId, url);
  }
  ...
  onTargetInfoChanged(event) {
    ...
    if (info) {
      ...
      // Page navigated from chrome:// to a real URL — re-inject recording script
      // (chrome://newtab has CSP that blocks page.evaluate)
      if (oldUrl && oldUrl.startsWith('chrome://') && !event.targetInfo.url.startsWith('chrome://') && info.page) {
        console.log(`[Wally Daemon] Page navigated: ${oldUrl.substring(0, 30)} → ${event.targetInfo.url.substring(0, 60)}`);
        info.page.addInitScript(RECORDING_SCRIPT).catch(e => log.debug(`addInitScript on navigate: ${e.message}`));
        info.page.evaluate(RECORDING_SCRIPT).then(() => {
          console.log(`[Wally Daemon] Re-injected recording: ${info.label}`);
        }).catch(e => log.debug(`evaluate on navigate: ${e.message}`));
      }
    }
  }
    ...
    try {
      ...
      if (targetPage) {
        ...
        if (isExtensionUrl(url)) {
          ...
          // Normal page — inject recording listeners
          try { await targetPage.addInitScript(RECORDING_SCRIPT); } catch (e) { console.log(`[Wally Daemon] addInitScript failed (${label}): ${e.message}`); }
          try { await targetPage.evaluate(RECORDING_SCRIPT); console.log(`[Wally Daemon] Initial injection OK: ${label}`); } catch (e) { console.log(`[Wally Daemon] Initial injection failed (${label}): ${e.message}`); }
        } else {
          // chrome:// page (newtab, new-tab-page) — will re-inject on navigation
          console.log(`[Wally Daemon] Chrome page (${label}) — recording starts on navigation`);
        }
      ...
      // "No target with given id found" means the tab was closed/reused before we could attach.
      // discoverPages() will pick it up via ctx.pages() polling if it still exists.
      if (e.message && e.message.includes('No target with given id')) {
        log.debug(`Attach deferred (target gone): ${url}`);
      } else {
  ...
  async discoverPages() {
    ...
    try {
      ...
      for (const p of pages) {
        const url = p.url();
        if (!url || url === "about:blank" || url.startsWith("chrome-devtools://")) continue;

        // Ext-aux mode: only discover OTHER extensions' targets
        if (this.extAux) {
          const isExt = url.startsWith("chrome-extension://");
          if (!isExt) continue;
          if (this.isWallyExtension(url)) continue;
        }

        // Skip pages that existed when daemon started (not part of this workflow)
        if (this.initialPages.has(url)) continue;
          ...
          for (const [targetId, info] of this.pages) {
            if (!info.page && info.url === url) {
              ...
              if (isExt) {
                ...
                try { await p.addInitScript(RECORDING_SCRIPT); } catch (e) { log.debug(`addInitScript discovered: ${e.message}`); }
                try { await p.evaluate(RECORDING_SCRIPT); } catch (e) { log.debug(`evaluate discovered: ${e.message}`); }
              }
      ...
      if (discovered > 0) {
        console.log(`[Wally Daemon] Discovery: ${discovered} new page(s), ${this.pages.size} total tracked`);
      }
    } catch (e) {
